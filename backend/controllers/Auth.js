...
exports.sendAdminOtp = async (req, res) => {
  try {
      ...
      if(!email){
          return res.status(500).json({
              success:false,
              message: "email Id is required for otp"
          })
      }
      ...
      if(!adminDetails){
      ...
      else if (adminDetails.email === email){
          var otp = otpGenerator.generate(6, {
              upperCaseAlphabets: false,
              lowerCaseAlphabets: false,
              specialChars: false,
          ...
          return res.status(500).json({
              success:false,
              message: "mail Id is not registerd for admin"
          })
      }
...
exports.adminSignUp = async (req, res) => {
  try {
      ...
      // console.log("body.signupData", req.body.signupData)
      ...
      return res.status(500).json({
          success: false,
          message: `admin is not registerd due to thie error ${error}`
      })
  }
};
...
exports.adminLogin = async (req, res) => {
  try {
      ...
      const adminDetails = await Admin.findOne({email: email}).select('firstName email lastName password accountType').exec();

      // console.log(adminDetails)
      if(!adminDetails){
          return res.status(500).json({
              success: false,
              message: "Admin is not registered for this email",
          })
      }
      ...
      if (await bcrypt.compare(password, adminDetails.password)) {
          const token = jwt.sign(
          { email: adminDetails.email, id: adminDetails._id, role: adminDetails.accountType },
          process.env.JWT_SECRET,
          {
              expiresIn: "24h",
          }
