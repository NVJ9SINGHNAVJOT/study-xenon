...
exports.getInstructorCourses = async (req, res) => {
  try {
    ...
    instructorCourses.map( async (course, index) => {
      let totalDuration = 0;
      course.courseContent.forEach((section) => {
        section.subSection.forEach((subSection) =>{
          const time = subSection.timeDuration.split(".");
          totalDuration += Number(time[0]);
        })
      })
     
      const tempObj = course.toObject();
      tempObj.completeDuration = `${Math.floor(totalDuration / 3600)} h ${Math.floor((totalDuration % 3600) / 60)} m`;

      instructorCoursesData.push(tempObj)
    })
...
exports.deleteCourse = async (req, res) => {
  try {
    ...
    // delete all ratings and reviews for that course
